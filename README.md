...
## Render me `==> Exited with status 1` aaye to kya check kare

Ye most common reasons hote hain:

1. **Missing env vars**
   - Required: `API_ID`, `API_HASH`, `BOT_TOKEN`, `XAPIVERSE_KEY`
   - Agar missing honge to app startup par exit karega.

2. **Invalid `API_ID` format**
   - `API_ID` numeric integer hona chahiye.

3. **Dependencies install issue**
   - Build logs me verify karo `pip install -r requirements.txt` successful hai.

4. **Pyrogram import failure**
   - Agar `ModuleNotFoundError: pyrogram` aaye, build command run nahi hua ya fail hua.

5. **Runtime test**
   - Deploy ke baad hit karo: `https://<render-domain>/health`
   - Agar 200 `{"status":"ok"}` aa raha hai to web process alive hai.

Quick checklist:
- Render dashboard -> Environment tab -> required vars set
- Deploy logs -> startup lines me configuration error to nahi
- Start command exactly: `python -m app.main`
